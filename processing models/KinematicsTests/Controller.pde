class Controller {
  ...
  static final int MANUAL_STATE = 0;
    ...
    switch(state) {
      case MANUAL_STATE:
        ...
        for(int i=0; i<LEG_COUNT; i++) {
          ...
          target.set(target.x * cos(currentMove.yaw) - target.y * sin(currentMove.yaw),
                     target.x * sin(currentMove.yaw) + target.y * cos(currentMove.yaw), target.z);
                     
          target.set(target.x, target.y * cos(currentMove.pitch) - target.z * sin(currentMove.pitch),
                               target.y * sin(currentMove.pitch) + target.z * cos(currentMove.pitch));                     

          target.add(currentMove.translation);
          target = legs[i].toLegSpace(target);
          legs[i].setTarget(target);
        }
        break;
      case LIFT_CMD_STATE:
        // Set new leg targets

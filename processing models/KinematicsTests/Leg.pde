class Leg {
  ...
  void draw(int view, float drawScale) {
    //if(parent.getLegNumber(this) > 1) return;
    switch(view) {
      case TOP_VIEW:
        pushMatrix();
          scale(drawScale);
  ...
  boolean setTarget(PVector p, boolean force) {
    ...
    if(force || canMoveTo(acts)) {
      //if(!canMoveTo(acts))
      //  p = new PVector(this.target.x, this.target.y, this.target.z - 10);
  ...
  public PVector getCenterTarget() {
        float[] acts = {actuators[0].midLength, actuators[1].midLength, actuators[2].midLength};
    PVector middle = solveFK(acts);
    return new PVector(middle.x, 0, parent.leg_up);
  }

...
public class leg_test extends PApplet {
...
public void draw() {
  ...
  case 3:  // top precise drawing mode
    ...
    pushMatrix();
      rotate(radians(-44.7f));
      line(0,0, 0,290+swingArm);
    popMatrix();
    pushMatrix();
      rotate(radians(33.8f));
      line(0,0, 0,290+swingArm);
    popMatrix();
...
public ActuatorSet IK(PVector foot) {
...
public PVector FK(ActuatorSet S) {
  ...
  switch (view) {
    ...
    case 3:
      if(mouseButton == LEFT)
        ...
        foot = new PVector(foot.x, foot.y, foot.z + (mouseY - pmouseY)/5.f);
      break;
    }
  writeSlowly(foot);
}

public void graphSample(float s) {
...
class ActuatorSet {
  ...
  public boolean isValid(ActuatorSet S) {
    return (S.a < max && S.a > min && S.b < max && S.b > min && S.c < max && S.c > min);
  }
}
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--stop-color=#cccccc", "leg_test" });
  }
}
